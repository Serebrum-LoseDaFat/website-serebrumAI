...
export const ideas: Idea[] = [
  ...
  },
  {
    ...
    motion: "PLG",
    rationale:
      "AI-native operating system for fractional CFOs (CFO-first; CMO, CPO/CTO/COO expand in Y2–Y3). Multi-client workspace where a fractional CFO uploads client financial data and AI builds an analytical ledger, generates board-quality deliverables — board updates, three-statement models, burn-rate dashboards, cap-table scenarios, KPI reports — per client per cadence. Plus billing, time tracking, and client pipeline mgmt. Brand name: Cortica (Latin form of 'cortex' — sister brand to SerebrumAI). PLG MOTION: 100% self-serve. No credit card at sign-up. 14-day trial, then subscribe. No free tier — the ICP is a professional evaluating a business tool, not a consumer kicking tires; a time-boxed trial with real stakes converts better than freemium for this buyer. DATA INGESTION (Y1): CSV and document import only — bank statement PDFs, QuickBooks/Xero CSV exports, Gusto/Rippling payroll CSV, Stripe revenue CSV, invoice exports, Carta cap table CSV. Every upload is CFO-controlled; no client OAuth required, no authorization chain, no waiting on anyone. AI categorizes transactions into an analytical ledger (explicitly a working model, not books of record — the distinction CFOs already make with their Excel models). Plaid bank sync and direct QuickBooks/Stripe integrations ship in Y2 as retention/upgrade features, not prerequisites. ACTIVATION PATH (target <10 min): sign up → create client workspace → upload 3 months of bank statements → AI builds ledger and dashboard (burn rate, runway, cash position, P&L approximation) → export first board update. CFO already has these files; zero client involvement at activation. UPGRADE TRIGGER: adding a second client hits the paywall → Pro ($299/mo, 5 clients). The CFO who adds a second client has already proven the first one worked — they convert without a sales conversation. PRICING: Pro $299/mo (up to 5 clients), Premium $499/mo (unlimited clients + peer benchmarks + client portal), Agency $999/mo (multi-fractional team collab). Math to $1M ARR: 280 paying subscribers × $299/mo avg × 12 = ~$1M — achievable Y1 with CFO-only focus. VIRAL LOOPS: (1) Client portal — give each client read-only dashboard access; founders see their burn rate live and tell peer founders 'my fractional CFO set this up for me' → new CFO acquisition channel. (2) Deliverable sharing — board decks and reports carry 'Made with Cortica' byline; each board meeting exposes 3–5 investors and board members. (3) LinkedIn content — role-specific deliverable templates as SEO/lead-gen; fractional CFOs share templates in community forums → inbound sign-ups. DATA FRESHNESS MECHANIC: without live integrations, weekly in-product nudge — 'Acme's data is 18 days old, upload this month's statements' — turns a limitation into an engagement mechanic. MARKET: 10–15K active fractional CFOs in US (growing 40–60% YoY), tight community (Bolster 10K+ members, Continuum, Catalant, On Deck CXO). ARPU $350/mo blended. SEQUENCING: CFO-only Y1 ($1–2M ARR) → CMO layer Y2 ($4–6M ARR, adds HubSpot/GA CSV) → CPO/CTO/COO Y3+ ($10–15M ARR) → fractional networks as customers Y4+ ($25–50M ARR). Y5 ceiling: $100–250M ARR realistic; >$500M with networks-as-customers tier. COMPETITIVE: no specialist tool for fractional CXOs exists. Adjacent but different ICP: Karbon/Pilot (accounting firm OS), Causal/Mosaic/Pry (FP&A for single-company teams, not multi-client fractionals), HoneyBook (broader service business), Notion/ClickUp (generic ops). True greenfield. MOATS: (1) cross-client anonymized benchmark database — Bloomberg-for-fractional-CFOs, materializes after 200+ users; (2) accumulated industry × role deliverable templates; (3) client portal creates two-sided switching costs; (4) peer referral network within tight community. SEREBRUM WEDGE: medium — studio uses fractionals; AlphaSigma portfolio companies hire fractionals; FedStat enterprise relationships often start via fractional engagements. RISKS: Notion AI ships role-specific fractional templates (25% prob in 12 mo); Bolster builds platform layer (20% in 18 mo — different team, real audience risk); generic AI makes deliverables trivially DIY (mitigated by multi-client data integration moat); fractional market growth slows (low — AI is increasing fractional demand by making it affordable). FMF requirement: HIGH — needs a founder who has been a fractional CFO. Med conviction: clear ICP, genuine pain, tight community, true greenfield, capital-efficient PLG path, multi-role expansion optionality; capped at $50–500M without aggressive expansion; execution risk is PLG activation quality and competitive window timing.",
  },
  {
    name: "Saven Tools",
    size: ">$500M",
    ...
    motion: "Hybrid",
    rationale:
      "Two-tool platform for software development teams: (1) Requirements Intelligence — AI extracts, structures, and traces requirements from unstructured sources (Slack channels, email threads, Jira/customer support tickets, AI-transcribed web meetings via Zoom/Meet/Teams) into a structured requirements registry with full source traceability; (2) QA Automation — AI generates test cases from those structured requirements and automates test execution across the SDLC. The critical differentiator is the closed loop: requirement extracted from a Slack thread → traceable to the test case that validates it → traceable back to the customer conversation that originated it. No competitor owns this end-to-end traceability chain. TOOL 1 — REQUIREMENTS INTELLIGENCE: Jira, Linear, Azure DevOps, and GitHub Issues all store requirements once a human writes them — none of them extract requirements from unstructured communication. Product managers spend 3–5 hours/week manually synthesizing Slack threads, support tickets, and meeting notes into specs. AI reads all sources, surfaces 'this customer asked for X in 3 different channels,' deduplicates, prioritizes by signal frequency, and writes the structured requirement with citations. Audit trail for compliance-sensitive teams (fintech, healthtech, govtech) is a secondary moat. TOOL 2 — QA AUTOMATION: AI reads the structured requirements registry and generates test cases, then scripts automated tests (Playwright/Cypress/Selenium output). Comps: mabl ($40M raised, AI-assisted testing), Testim.io (acquired by Tricentis ~$80M, AI test creation), Rainforest QA (~$20M ARR, crowd + AI), Katalon (~$45M raised), Applitools (visual AI). AI-native QA automation is proven to exist — the differentiation here is generation from structured requirements (not from code or UI alone), so tests reflect product intent, not just current behavior. MARKET: Requirements management is a $1B+ niche within the $30B+ software development tools market; QA automation is a $15–25B market growing 18% CAGR. Combined AI-native DevTools ceiling: Atlassian ($4B ARR), GitHub ($2B+ ARR), Tricentis ($3B valuation). Saven's niche — AI requirements → AI tests with traceability — has no dominant incumbent. >$500M ARR ceiling plausible if it becomes the AI-native requirements + QA layer for mid-market engineering teams. PATH TO $1M ARR: Hybrid PLG — developers and PMs self-serve (free trial → Pro $79/user/mo or team plan $499/mo for 10 seats), then enterprise motion for orgs with compliance traceability requirements ($2K–10K/mo). 200 team-plan customers = $1.2M ARR; 12–18 months via focused engineering community GTM. TOKEN SOURCE WEDGE: same buyer orbit — VP Engineering, Head of Product, QA Lead at software companies. TokenSource's enterprise engineering relationships are a direct warm-intro pipeline. DISTRIBUTION: developer community content (the requirements extraction demo — 'paste your Slack thread, get a structured spec in 30 seconds' — is inherently shareable on LinkedIn/X/HN); integration marketplace listings (Jira App, Slack App, GitHub App accelerate inbound discovery); conference presence at QA/DevOps events (QA Global, StarEast, DevOpsDays). MOATS: (1) traceability database — after 12 months of data, Saven knows which requirement sources produce the highest-quality specs and which test patterns catch the most bugs; (2) integration depth — Slack + Jira + Zoom + Confluence + GitHub in one workflow is painful to replicate; (3) requirements template library per industry vertical (fintech compliance, healthtech HIPAA, govtech security). RISKS: GitHub Copilot expands into requirements/test generation (high probability in 18–24 mo, but Copilot is code-first, not requirements-first — different entry point); Atlassian Intelligence adds requirements extraction to Jira (real risk — they own the ticket; mitigated by multi-source extraction being architecturally complex to bolt onto Jira); individual tools (requirements-only or QA-only) out-PLG the bundle (real — keep tools independently purchasable, sold as a platform). FMF requirement: HIGH — needs a founder from product management or QA engineering. Med conviction: genuine unsolved pain, clear buyer, TokenSource distribution wedge, proven QA automation market with AI-native opportunity; capped below max conviction by Atlassian/GitHub platform risk and the execution complexity of two tools.",
  },
  {
    name: "Vitasana.ai",
    size: "$50M–$500M",
